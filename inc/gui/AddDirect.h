...
class AddDirect : public QDialog {
public:
    QGridLayout* layout;
    QLineEdit* name{};
    QDoubleSpinBox* xDir;
    ...
    void acceptfunc() {
        ...
        auto y = yDir->value();
        auto z = zDir->value();
        DirectionalLight light(Vector<3, double>(x, y, z).normalize());
        light.name = name->text().toStdString();
        auto r = rColor->value();
        auto g = gColor->value();
        auto b = bColor->value();
        light.color() = {r, g, b};
        scene.lights().directional().push_back(light);
        accept();
    };
    explicit AddDirect(Scene& s) : scene(s) {
